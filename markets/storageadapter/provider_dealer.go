...
func NewProviderNodeAdapterDealer(dag dtypes.StagingDAG, sealingApi api.StorageMiner, full api.FullNode) storagemarket.StorageProviderNode {
	return &ProviderNodeAdapterDealer{
		FullNode:   full,
		dag:        dag,
...
func (n *ProviderNodeAdapterDealer) EnsureFunds(ctx context.Context, addr, wallet address.Address, amt abi.TokenAmount, encodedTs shared.TipSetToken) (cid.Cid, error) {
...
func (n *ProviderNodeAdapterDealer) OnDealSectorCommitted(ctx context.Context, provider address.Address, dealID abi.DealID, cb storagemarket.DealSectorCommittedCallback) error {
...
func (n *ProviderNodeAdapterDealer) GetDataCap(ctx context.Context, addr address.Address, encodedTs shared.TipSetToken) (*verifreg.DataCap, error) {
	tsk, err := types.TipSetKeyFromBytes(encodedTs)
	if err != nil {
		return nil, err
	}
