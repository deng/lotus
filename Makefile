...
debug: lotus lotus-miner lotus-worker lotus-seed lotus-sealer lotus-dealer

2k: GOFLAGS+=-tags=2k
2k: lotus lotus-miner lotus-worker lotus-seed lotus-sealer lotus-dealer
...
lotus-sealer: $(BUILD_DEPS)
	rm -f lotus-sealer
	go build $(GOFLAGS) -o lotus-sealer ./cmd/lotus-sealer
	go run github.com/GeertJohan/go.rice/rice append --exec lotus-sealer -i ./build
.PHONY: lotus-sealer
BINS+=lotus-sealer
